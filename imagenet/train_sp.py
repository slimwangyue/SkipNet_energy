...
def parse_args():
    ...
    parser.add_argument('--epochs', default=80, type=int,
                        help='number of total epochs (default: 80)')
    ...
    parser.add_argument('--save-folder', default='save_checkpoints_sp', type=str,
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # if energy_ratios.len != len(energy_parameter):
        #     energy_ratios.set_len(len(energy_parameter))
        if args.energy:
            loss = criterion(output, target_var) + energy_cost.sum()
        else:
        ...
        if i % args.print_freq == 0:
            ...
            # logging.info('*** Energy Percentage: {:.3f} %'.format(cp_energy))
...
def validate(args, val_loader, model, criterion, epoch):
    ...
    logging.info(' * Prec@1 {top1.avg:.3f}\tPrec@5 {top5.avg:.3f}\t'
                 'Loss {loss.avg:.3f}\t'
                 'Energy_ratio: {cp_energy_record.avg:.3f}\t'
                 .format(top1=top1, top5=top5, loss=losses, cp_energy_record=cp_energy_record))
